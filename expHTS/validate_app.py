...
class validateApp:
	...
	def validateSampleSheet(self, dirSample, sampleSheet):
		linenum = 0
		...
		for e in f.readlines():
			if linenum != 0:
				self.sampleSequenceID(dirSample, e.split("\t"))
			linenum += 1
	...
	def sampleSequenceID(self, dirSample, seqID):
		if seqID[0][0] == "#":
			pass
		elif len(seqID) >= 2:

			seqID[0] = os.path.join(dirSample, seqID[0])
			seqID[1] =  seqID[1].rstrip()
